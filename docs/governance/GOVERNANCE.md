...
- `ACCEPTED`: the default path requires an independent approval or `/verify PASS <full-head-sha>` record from a trusted collaborator that matches the current Pull Request HEAD. A personal repository may explicitly choose the solo path with `/verify SOLO PASS <full-head-sha> EVIDENCE <codex-or-https-reference>`; this records `verification:solo-owner` and is an owner attestation, not a second human reviewer.
