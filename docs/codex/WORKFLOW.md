...
高风险正常流转：DRAFT -> ANALYZING -> READY -> IN_PROGRESS -> READY_FOR_VERIFY -> ACCEPTED -> CLOSED。只有由治理工具按显式白名单确定为 L1 的普通研究/功能代码，才可使用 `DRAFT`、`ANALYZING` 或 `READY` -> `IN_PROGRESS` -> `READY_FOR_VERIFY` -> `ACCEPTED` -> `CLOSED` 的紧凑路径；分类不明时按高风险处理。
分析、实施或验收遇到关键事实、权限或环境不足时记录 BLOCKED；验收失败为 REWORK，返工再进入 IN_PROGRESS。高风险或分类不明时，恢复前重新核对阻塞原因与任务契约，不跳过 READY；仅显式 L1 紧凑路径可跳过前置分析状态。
